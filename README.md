...
Content Production Factory 是基于阿里云百炼和 FFmpeg 的内容生产 Skill 和工具集，覆盖视频剪辑（FFmpeg 直接调用）、视频音轨无损抽取、文件上传与 URL 管理、语音识别、声音复刻、语音合成、视觉理解、Qwen-OCR 和 AI 图片生成。

项目目前通过 `scripts/main.py` 统一提供 7 个命令组、31 个子命令，同时保留原有独立入口。所有模型命令默认使用当前已验证的最高质量参数；只有明确指定时才切换到 Flash、关闭思考或降低图像分辨率。

## 能力概览

| 能力 | 主要功能 | 默认模型/工具 |
|---|---|---|
| 视频剪辑 | 裁剪、拼接、缩放、调速、滤镜、字幕烧录、转场、调色、画中画 | FFmpeg 8.1.2，直接调用 CLI |
| 本地媒体处理 | 从视频无损抽取音轨；兼容编码原样复制，不兼容时转 FLAC | PyAV，本地执行，不使用 AI |
...
| 图片生成 | 文生图、图生图、图片编辑和 3D 生成，支持 12 款模型、v1 异步和 v2 同步接口 | `gpt-image-2` |
...
```powershell
python -m pip install -U av click dashscope requests
```
...
| 图片生成 | `gpt-image-2`、1K、high 质量、v2 同步 | `--quality low` 或低价模型；升级分辨率用 `--resolution 4K` |
...
无 Python 封装，直接使用 FFmpeg CLI。完整命令参考 [references/ffmpeg.md](references/ffmpeg.md)。

| 操作 | 命令示例 |
|---|---|
| 截取片段 | `ffmpeg -i input.mp4 -ss 10 -to 30 -c copy output.mp4` |
| 拼接 | `ffmpeg -f concat -safe 0 -i files.txt -c copy output.mp4` |
| 缩放 | `ffmpeg -i input.mp4 -vf scale=1920:-2 output.mp4` |
| 变速 | `ffmpeg -i input.mp4 -filter_complex "[0:v]setpts=0.5*PTS[v];[0:a]atempo=2.0[a]" output.mp4` |
| 画中画/水印 | `ffmpeg -i main.mp4 -i logo.png -filter_complex "overlay=20:20" output.mp4` |
| 字幕烧录 | `ffmpeg -i video.mp4 -vf "subtitles=subs.srt" output.mp4` |
| 转场 | `ffmpeg -i a.mp4 -i b.mp4 -filter_complex "xfade=fade:d=1" output.mp4` |

> 首次使用需安装：`winget install ffmpeg`。运行 `ffmpeg -version` 检查是否已装。

### 本地媒体处理

统一入口：`python scripts/main.py media`

| 命令 | 用途 |
|---|---|
| `extract-audio` | 最高保真抽取视频音轨，优先原样复制，不兼容时转无损 FLAC |
...
统一入口：`python scripts/main.py image`

| 命令 | 用途 |
|---|---|
| `generate` | v2 同步文生图，直接返回图片数据（推荐） |
| `generate-async` | v1 异步文生图，返回 task_id 并可选轮询下载 |
| `edit` | v2 同步图片编辑，上传图片按提示词修改 |
| `query-task` | 查询异步任务状态，完成后自动下载 |
| `list-models` | 列出所有图片模型和价格 |
...
```powershell
python scripts/main.py media extract-audio "runtime/inputs/video.mp4"
```

AAC、MP3、FLAC、Opus 和 Vorbis 优先执行数据包级复制，不解码、不重编码；其他编码自动转为保留原采样率和声道布局的 FLAC。默认输出到 `runtime/outputs/<视频名>-audio.<扩展名>`。
...
```powershell
python scripts/main.py image generate --prompt "一只可爱的猫咪在弹钢琴" --size "1:1" --output "runtime/outputs/cat.png"
```

### 高分辨率文生图

```powershell
python scripts/main.py image generate --prompt "超高清宇宙星空图" --size "16:9" --resolution "4K"
```

### 参考图生图

```powershell
python scripts/main.py image generate --model "nano-banana-fast" --prompt "转为水彩画风格" --size "1:1" --image "runtime/inputs/photo.jpg"
```

### v1 异步文生图

```powershell
python scripts/main.py image generate-async --prompt "赛博朋克城市夜景" --size "16:9" --resolution "2K"
```
...
- `visual analyze-video` 只分析视频画面，不识别音轨；视频口播先用 `media extract-audio`，再调用 `speech` 命令。
...
- 首次打包只有在用户明确要求时执行。
- 图片生成使用麦子科技 API，通过 `python scripts/main.py key set --service maizi <密钥>` 配置。
- v1 异步生成的签名 URL 和 Seed3D ZIP 下载链接 24 小时有效，请及时下载。

...
项目目前通过 `scripts/main.py` 统一提供 4 个命令组、22 个子命令，同时保留原有独立入口。所有命令默认使用当前已验证的最高质量模型和参数；只有明确指定时才切换到 Flash、关闭思考或降低图像分辨率。

## 能力概览

| 能力 | 主要功能 | 默认模型 |
|---|---|---|
| 语音识别 | 本地音视频离线转写与 SRT、短音频、12 小时长音频、情感、时间戳、热词、上下文增强、说话人分离、敏感词过滤 | 本地默认 `large-v3`；百炼按场景使用 `qwen3-asr-flash`、`qwen3-asr-flash-filetrans`、`fun-asr` |
...
```powershell
python -m pip install -U click dashscope faster-whisper requests
```
...
| 命令 | 用途 |
|---|---|
| `recognize` | 识别本地短音频，返回文本、情感和语言 |
| `recognize-context` | 使用上下文增强识别准确率 |
| `transcribe-local` | 使用本地 Whisper 模型识别音频或视频并生成 SRT |
| `transcribe-long` | 转写最长 12 小时的公网音频，返回句级或字级时间戳 |
...
```powershell
python scripts/main.py speech transcribe-local "runtime/inputs/audio.mp3" --output "runtime/outputs/audio.srt"
```

默认使用最高质量 `large-v3`。首次运行需要下载模型；硬件资源不足时可明确使用 `--model large-v3-turbo --device cpu --compute-type int8`。
...
- 本地 SRT 使用 `faster-whisper`，不需要百炼密钥；首次运行需要联网下载模型，之后使用本地缓存。
- `large-v3` 是本地字幕默认最高质量模型，对内存和显存要求较高；低资源设备需明确选择较小模型。

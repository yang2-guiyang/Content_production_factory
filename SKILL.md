...
description: 提供语音识别、本地音视频离线转写与 SRT 字幕导出、Qwen-Audio-TTS 声音复刻与非实时语音合成、视觉理解和 Qwen-OCR CLI。用于用户需要识别短音频或长音频、从本地媒体生成字幕、获取情感和时间戳、使用热词或上下文增强、执行说话人分离和敏感词过滤，使用声音样本创建或管理复刻音色、把文本合成为语音、控制合成情感语速方言或拟声效果，以及分析单图、多图、视频或视频帧，完成图片描述、视觉问答、文字定位、票据证照信息抽取、表格解析、公式识别、多语言 OCR、扫描文档或 PDF 解析、物体定位、创意写作和视频事件理解时。
---
...
3. 调用百炼模型前，使用密钥状态命令确认 `DASHSCOPE_API_KEY` 已配置；纯本地 `speech transcribe-local` 不需要密钥。
...
- 本地音频或视频离线转写并导出 SRT 字幕：使用 `speech transcribe-local`。
- 百炼语音转文字、长音频、时间戳、情感、热词、上下文增强、说话人分离或敏感词过滤：使用 `speech` 命令组中的其他命令。

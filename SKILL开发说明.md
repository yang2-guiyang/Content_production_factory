...
> **更新记录**（近 5 条）
> - 2026-07-18 v5：统一 CLI.md 编写规范；取消简写版 CLI.md，要求已创建的 CLI.md 按实际 `--help` 结果逐条完整展开用途、完整语法、参数表和示例；v5.1：删除 SKILL.md 正文中重复的“激活条件”，触发场景只写在 frontmatter 的 `description` 中；v5.2：细化九步开发流程，增加每步进入条件、执行范围、验证标准、交付物和强制停点
...
九个 Step 必须严格串行执行。AI 在任何情况下都不得在同一轮中完成两个或更多 Step。
...
**强制停点**：展示四步自发现结果，询问用户是否进入 Step 5，然后停止。

#### 3.1.6 Step 5：生成 spec 并打包 exe

**目标**：将已验证的 Python CLI 打包为 Windows 可执行文件。

**进入条件**：Step 4 已通过，用户已确认进入 Step 5。
...
**禁止事项**：本 Step 只验证当前开发环境中的 exe，不得把它描述为已通过干净环境验证。

**强制停点**：展示 exe 路径、大小和基础验证结果，询问用户是否进入 Step 6，然后停止。

#### 3.1.7 Step 6：在干净环境验证 exe

**目标**：证明发布物在没有源码和 Python 依赖的环境结构中可以独立运行。

**进入条件**：Step 5 已成功生成 exe，用户已确认进入 Step 6。
...
**进入条件**：Step 6 已通过，用户已确认进入 Step 7。
...
**强制停点**：逐篇汇报验证结果，询问用户是否进入 Step 9，然后停止。

#### 3.1.10 Step 9：打包发布

**目标**：生成不含源码和密钥的最终 Skill 发布包。

**进入条件**：Step 8 已通过，用户已确认进入 Step 9。

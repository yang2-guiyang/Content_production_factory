...
> **打包默认规则**：用户未明确要求打包时，禁止生成 `.spec`、禁止运行 PyInstaller、禁止创建或验证 exe、禁止生成发布压缩包。打包相关的 Step 5、Step 6 和 Step 9 均为可选步骤，只能由用户主动明确要求后进入；不得把“是否继续”自动解释为打包授权。
...
一个 Skill 文件包是把一项 AI 能力封装成的“Python CLI + SKILL.md + 文档（按需）+ SOP 手册”套装。Python CLI 和 SKILL.md 是必有项，exe、CLI.md、SOP 手册和发布压缩包均按需配置。用户明确要求免 Python 分发时，才把 Python CLI 打包为 exe。

四层文件的分工：

| 文件 | 层 | 解决什么问题 | 谁来写 |
|------|-----|-------------|--------|
| `scripts/main.py` / `scripts/xxx.exe`（按需） | 执行层 | 实际干活的工具。源码 CLI 必须支持四步自发现；仅在用户明确要求时额外生成 exe | 开发者 |
...
> **命名约定**：文件组织遵循 skill-creator 标准——可执行代码放在 `scripts/` 下；只有用户明确要求打包时才合并为 exe。CLI.md 和 SOP 手册统一放在 `references/` 目录下。
...
> **用途限制**：以下流程图仅供 AI 判断当前处于哪个 Step，不是自动执行清单。AI 只能为用户已经明确确认的当前 Step 制定计划和执行操作；禁止提前规划全部 Step，禁止为未确认的后续 Step 创建待办、排期或执行队列，禁止完成当前 Step 后自动进入下一 Step。用户回复“继续”时，最多只授权进入紧邻的下一个必需 Step；可选 Step 仍需用户明确说出对应操作。
...
Step 4: 写 main.py
        ...
        │
        ▼
Step 5（可选）: 生成 .spec + 打包 exe
        仅用户明确要求打包时进入
        pyi-makespec → 改 name → pyinstaller
        │
        ▼
Step 6（可选）: 在干净环境验证 exe 能跑
        仅 Step 5 已按用户要求执行时进入
        │
        ▼
...
开发 Step 必须严格串行执行。AI 在任何情况下都不得在同一轮中完成两个或更多 Step，也不得一次性规划全部 Step。Step 5、Step 6 和 Step 9 默认跳过，只有用户主动明确要求对应打包或发布操作时才进入。

每个 Step 固定按以下顺序执行：

1. **进入检查**：确认上一 Step 已验证通过，并且用户已明确同意进入当前 Step。没有用户确认时，不得为该 Step 创建计划或待办。
2. **单项实施**：只规划和实施当前 Step，只创建或修改当前 Step 明确要求的文件，不提前规划后续 Step，不提前创建后续 Step 的产物。
3. **逐项验证**：一个 Step 包含多个脚本或检查项时，完成一个就立即验证一个；验证失败先修复，不得带着失败继续。
4. **Git 提交**：当前 Step 验证通过后，使用中文提交说明创建一个独立提交。
5. **结果汇报**：向用户展示新增或修改的文件、实际运行命令、关键输出、验证结果和提交 ID。
6. **强制停止**：明确询问是否进入下一 Step，然后结束当前轮。只有用户明确回复继续，才能开始下一 Step。

以下行为一律禁止：

- 在 Step 1 同时创建 Click 命令、`.env`、`main.py` 或文档
- 在 Step 2 提前实现鉴权、打包或 SKILL.md
- 用户未明确要求时生成 `.spec`、exe 或发布压缩包
- 把多个未确认 Step 写进同一个计划、待办或执行队列
- 当前 Step 完成后自动规划或执行下一 Step
- 未运行真实命令就宣称验证通过
- 当前 Step 测试失败时继续下一个 Step
- 用户改变模型、接口或功能范围后继续沿用旧验证结果

如果用户中途改变模型、API、输入方式或功能范围，必须退回最早受影响的 Step，重新实施并验证。例如模型从 Qwen-TTS 改为 Qwen-Audio-TTS，必须重新执行 Step 1 的真实接口验证，再重新执行 Step 2 的 CLI 封装。
...
**强制停点**：汇报真实验证结果，询问用户是否进入 Step 2，然后停止。

#### 3.1.3 Step 2：拆分 Click 命令组

**目标**：把 Step 1 已验证的单一能力转换为清晰、可发现的 CLI 接口。

**进入条件**：Step 1 已真实运行成功，用户已确认进入 Step 2。
...
**禁止事项**：不得新增 Step 1 未验证的模型、API 或功能；不得提前写 `.env`、`main.py` 或文档。

**强制停点**：展示 CLI 设计和实际执行结果，询问用户是否进入 Step 3，然后停止。

#### 3.1.4 Step 3：实现 .env 鉴权

**目标**：让外部 API 密钥从 `scripts/.env` 安全读取和维护。

**进入条件**：Step 2 已通过，用户已确认进入 Step 3；纯计算型 Skill 可申请跳过。

**API 型 Skill 只做以下工作**：

1. 创建 `scripts/.env`，写入当前 Skill 实际需要的键名
2. 将 `scripts/.env` 加入 `.gitignore`
3. 创建只读模块，按 UTF-8 逐行读取，使用 `split("=", 1)`
4. 创建独立写入命令模块，提供状态查询、写入和删除能力
5. 状态查询只显示是否配置，不回显密钥
6. 修改业务命令，使其只通过读取模块取得密钥
7. 同时验证源码模式和预期的 PyInstaller 路径逻辑
...
**纯计算型跳过规则**：AI 必须说明不存在外部鉴权需求，取得用户确认后记录“Step 3 已确认跳过”，不得自行跳过。

**强制停点**：只报告配置状态，不展示密钥；询问用户是否进入 Step 4，然后停止。

#### 3.1.5 Step 4：编写 main.py

**目标**：提供唯一 CLI 入口，并完成 AI 四步自发现能力。

**进入条件**：Step 3 已完成或已由用户确认跳过，用户已确认进入 Step 4。

**只做以下工作**：

1. 创建 `scripts/main.py`
2. 在第三方库导入前强制 stdout 使用 UTF-8
3. 显式导入并注册现有命令组，不做动态发现
4. 实现 `list-groups`，以 JSON 返回命令组名称和描述
5. 实现 `list-commands <group>`，以 JSON 返回子命令名称和描述
6. 保持源码入口格式为 `python scripts/main.py <组> <命令> [参数]`
...
**禁止事项**：不得开始 PyInstaller 打包，不得提前写 SKILL.md、CLI.md 或 SOP。

**强制停点**：展示四步自发现结果。用户此前未明确要求打包时，询问是否进入 Step 7 文档阶段并停止，不得主动进入或推荐 Step 5；只有用户已明确要求打包时，才询问是否进入 Step 5。

#### 3.1.6 Step 5（可选）：生成 spec 并打包 exe

**目标**：将已验证的 Python CLI 打包为 Windows 可执行文件。

**进入条件**：Step 4 已通过，并且用户主动明确要求生成 exe。普通的“继续”或“进入下一步”不构成打包授权。

**只做以下工作**：

1. 运行 `pyi-makespec scripts/main.py` 生成 spec
2. 按项目命名规范修改 exe 名称
...
**禁止事项**：用户未明确要求打包时不得进入本 Step。本 Step 只验证当前开发环境中的 exe，不得把它描述为已通过干净环境验证。

**强制停点**：展示 exe 路径、大小和基础验证结果，询问用户是否进入 Step 6，然后停止。

#### 3.1.7 Step 6（可选）：在干净环境验证 exe

**目标**：证明发布物在没有源码和 Python 依赖的环境结构中可以独立运行。

**进入条件**：用户已明确要求打包，Step 5 已成功生成 exe，并且用户已确认进入 Step 6。
...
**强制停点**：展示干净环境证据，询问用户是否进入 Step 7，然后停止。

#### 3.1.8 Step 7：编写 SKILL.md 和 CLI.md

**目标**：让 AI 能根据用户意图激活 Skill、选择命令并准确构造参数。

**进入条件**：未要求打包时 Step 4 已通过；要求打包时 Step 6 已通过。用户已确认进入 Step 7。

**只做以下工作**：

1. 编写根目录 `SKILL.md`
2. frontmatter 只保留 `name` 和 `description`
3. 所有触发场景只写入 `description`，正文不重复“激活条件”
4. 正文编写互不重叠的场景路由和决策逻辑
5. 根据 §十.1 判断是否需要 `references/CLI.md`
6. 需要 CLI.md 时，按真实 `--help` 逐条完整展开用途、语法、参数表和已验证示例
7. 小项目不创建 CLI.md 时，在 SKILL.md 中保留四步自发现和命令速查
8. 运行 skill-creator 的 `quick_validate.py`
...
**强制停点**：展示路由和命令文档摘要，询问用户是否进入 Step 8，然后停止。

#### 3.1.9 Step 8：编写 SOP 手册

**目标**：用场景化操作文档覆盖用户最常见的完整任务。

**进入条件**：Step 7 已通过，用户已确认进入 Step 8。
...
**强制停点**：逐篇汇报验证结果后停止。用户未明确要求生成发布包时，不得主动进入或推荐 Step 9；用户已明确要求发布时，才询问是否进入 Step 9。

#### 3.1.10 Step 9（可选）：打包发布

**目标**：生成不含源码和密钥的最终 Skill 发布包。

**进入条件**：Step 8 已通过，用户主动明确要求生成发布包，并已确认进入 Step 9。

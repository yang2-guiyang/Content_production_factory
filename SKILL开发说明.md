...
> **更新记录**（近 5 条）
> - 2026-07-18 v5：统一 CLI.md 编写规范；取消简写版 CLI.md，要求已创建的 CLI.md 按实际 `--help` 结果逐条完整展开用途、完整语法、参数表和示例；v5.1：删除 SKILL.md 正文中重复的“激活条件”，触发场景只写在 frontmatter 的 `description` 中；v5.2：细化九步开发流程，增加每步进入条件、执行范围、验证标准、交付物和强制停点；v5.3：打包改为用户明确要求后才执行的可选流程
...
开发 Step 必须严格串行执行。AI 在任何情况下都不得在同一轮中完成两个或更多 Step。Step 5、Step 6 和 Step 9 默认跳过，只有用户主动明确要求对应打包或发布操作时才进入。

每个 Step 固定按以下顺序执行：

1. **进入检查**：确认上一 Step 已验证通过，并且用户已明确同意进入当前 Step。
2. **单项实施**：只创建或修改当前 Step 明确要求的文件，不提前创建后续 Step 的产物。

...
| `CLI.md` | 参考层（按需） | 命令参数快照——四步流程的预计算结果。大项目独立建，中项目简写，小项目并入 SKILL.md | 开发者（按 §十.1 判断） |
...
- **CLI.md**（按需）：命令参数快照——四步流程的预计算结果。大项目独立建文件省 AI 往返，中项目简写骨架，小项目并入 SKILL.md
...
| 中型项目（4-10 组） | **简写** | 四步流程 + 命令骨架表 + 通用参数，不展开参数细节 |
| 大型项目（>10 组或 >50 子命令） | **完整** | 四步流程 + 全部命令 + 参数表 + 注意事项 |
...
### 10.1 咒语表（CLI 命令参考）— 可简写、可并入 SKILL.md

**定位**：告诉 AI「有哪些命令、参数怎么填」。纯技术参考，不包含场景选择逻辑。文件名为 `CLI.md`，放在 `references/` 目录下。

**核心设计**：CLI.md 的第一节必须是「四步自发现流程」——这是 AI 学习工具的固定路径。文档的核心价值是告诉 AI "怎么学"，而非罗列所有细节。参数细节由 AI 在 Step 3 用 `--help` 动态获取，文档只需给出四步流程 + 命令骨架。

#### 是否需要 CLI.md？

| 项目规模 | 做法 | 原因 |
|---------|------|------|
| **小项目**（≤3 个命令组，≤15 个子命令） | **不写 CLI.md**，命令直接列在 SKILL.md 的「## 可用命令」章节 | 减少上下文加载量，AI 一次加载 SKILL.md 即可掌握全部命令 |
| **中型项目**（4-10 个命令组） | **简写 CLI.md**：四步流程 + 命令骨架表，不展开参数细节 | AI 先走四步流程掌握全局，参数用 `--help` 按需查询 |
| **大型项目**（>10 个命令组或 >50 个子命令） | **完整 CLI.md**：四步流程 + 全部命令展开 + 参数表 + 注意事项 | 命令量大，逐条 `--help` 成本高，预置完整文档省往返 |

> **权衡原则**：能靠 `--help` 动态获取的信息不写进 CLI.md，能并入 SKILL.md 的就不单独建文件。目标是让 AI 用最少的上下文加载量掌握工具用法。命令表本质上是"四步流程的预计算快照"——不是必须品，是加速包。

#### 四步自发现流程（CLI.md / SKILL.md 命令章节第一节，必须）

无论项目大小、CLI.md 是否存在，**命令参考的第一节必须是四步流程**。这是 AI 学工具的最短路径，把"教学路径"固化在文档开头：

```markdown
## AI 自发现流程（四步掌握全部命令）

拿到工具后，AI 按以下四步自行发现全部能力，无需依赖外部文档：

\```
Step 1 — 查看有哪些能力模块
  exe list-groups
  → JSON：[{"name": "data", "description": "数据处理命令组"}, ...]

Step 2 — 查看模块下有哪些子命令及用途
  exe list-commands <group>
  → JSON：[{"name": "filter", "description": "按条件筛选数据"}, ...]

Step 3 — 查看子命令的详细帮助（参数、默认值、使用示例）
  exe <group> <command> --help
  → Click 自动输出完整帮助文本（必填参数、可选参数、默认值）

Step 4 — 执行命令
  按 Step 3 获取的参数信息构造命令并执行
\```

> AI 拿到 exe 后走一遍这四步即可掌握全部命令。本章后续的命令表是快捷参考——四步流程的运行结果快照。
```

> **注意**：四步流程中，Step 1 和 Step 2 用 `list-groups` / `list-commands`（JSON 输出，AI 解析零误差），Step 3 用 Click 原生的 `--help`（人类友好的格式化文本）。双轨制——机器读 JSON，人类读 --help。

#### 简写版模板（中小项目，推荐）

四步流程 + 命令骨架表，不展开参数。参数细节由 AI 在 Step 3 用 `--help` 动态获取。
...
| 环境 | 入口 | 格式 |
|------|------|------|
| 开发 | `python scripts/main.py` | `python scripts/main.py <组> <命令> [参数]` |
| 生产 | `scripts/<name>.exe` | `scripts\<name>.exe <组> <命令> [参数]` |

## 命令组一览

| 命令组 | 子命令数 | 说明 |
|--------|:-------:|------|
| `data` | 3 | 数据处理：filter / export / transform |
| `admin` | 2 | 管理功能：list / add |

> 子命令列表和参数用法通过 `exe list-commands <组>` 和 `exe <组> <命令> --help` 动态获取。

## 通用参数

- `<路径>`：替换为实际文件路径，支持相对路径和绝对路径
- `--out`：可选，输出文件路径。不填则输出到控制台
```

#### 完整版模板（大型项目）

四步流程 + 全部命令展开 + 参数表。适用于命令量大、逐条 `--help` 成本高的场景。
...
\```
scripts\<name>.exe data filter --input <路径> [--out <路径>]
\```

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:---:|--------|------|
| `--input` | PATH | ✓ | - | 输入文件路径 |
| `--out` | PATH | ✗ | 控制台 | 输出 JSON 文件路径 |
...
## 注意事项
- 输入文件支持通用格式（根据 Skill 实际能力填写）
- 单个文件不超过特定大小限制（根据 Skill 实际能力填写）
- ...
```

#### 小项目 SKILL.md 嵌入写法

小项目不建 CLI.md，将命令直接写在 SKILL.md 中，减少 AI 上下文加载量：

```markdown
## 可用命令

以下命令可通过四步自发现动态查询（`exe list-groups` → `exe list-commands <组>` → `exe <组> <命令> --help`），此处为快捷参考。

### 命令组一览

| 命令组 | 说明 | 子命令 |
|--------|------|--------|
| `data` | 数据查询 | search / list / cats / run |
| `calc` | 量化计算 | metrics / portfolio / factor-mine / factor-backtest / factor-mgr / factor-apply |

### 最常用命令

\```
python scripts/main.py data search <关键词> [--head N]    # 搜索函数
python scripts/main.py data run <函数名> [k=v ...]        # 执行查询
python scripts/main.py calc metrics --nav-file <路径>      # 单资产指标
python scripts/main.py calc portfolio --nav-files <路径>   # 组合优化
\```

> 完整参数用法：`python scripts/main.py <组> <命令> --help`
```

#### 编写要点

- **四步流程必须作为第一节**——这是 AI 学工具的最短路径，固化"教学路径"
- **简写版不写参数表**——参数让 AI 在 Step 3 用 `--help` 动态获取，避免文档与代码不同步
- **完整版参数表是给人类速查用的**——AI 依然走四步流程，参数表是备用参考
- 命令格式用尖括号 `<>` 标注用户可替换部分
- 不要写技术实现细节（模型名、API 端点等）
- 不要写场景选择逻辑（那是 SKILL.md 的事）
- 通用参数统一列出，避免每条命令重复写
